...
const ExperiencesTimeline = () => {
    return (
        <div id="Experience" className="sm:py-10 py-5  flex flex-col animate-fade-right">
          <h2 className="font-semibold tracking-tighter sm:text-3xl flex sm:mb-10 mb-5 sm:justify-center text-2xl">
            ...
            {experience.map(item => (
                <div key={item.title} className="relative px-3 flex sm:grid grid-cols-3">
                  <p className="grid justify-end absolute sm:relative bottom-2 font-bold right-0 sm:mr-6 mt-3 " translate="no">
                        {item.from} - {item.to}
                  </p>
